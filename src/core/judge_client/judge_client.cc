...
static char lang_ext[19][8] = { "c", "cc", "pas", "java", "rb", "sh", "py",
		"php", "pl", "cs", "m", "bas", "scm","c","cc","lua","js","go" ,"jar"
    };
...
void init_syscalls_limits(int lang) {
	...
	if (record_call) { // recording for debuging
		...
		for (i = 0; i==0||LANG_JARV[i]; i++)
			call_counter[LANG_JARV[i]] = HOJ_MAX_LIMIT;
	}
...
//                	if(lang>2 && lang!=10 && lang!=13 && lang!=14)execute_cmd("mount -o bind /dev dev");
                        chroot(work_dir);
		}
...
void _get_solution_mysql(int solution_id, char * work_dir, int lang) {
    //ToDo Fuck it
    // add copy solution file to run dir
	...
	if(lang != 18){
		...
		sprintf(src_pth, "/bin/cp %s/jars/%d.jar ./Main.jar", oj_home, solution_id);
		if(DEBUG)
			printf("Jar Lang is Detected !\n");
		execute_cmd(src_pth);
	}
}
...
//      char java_p1[BUFFER_SIZE], java_p2[BUFFER_SIZE];
    ...
    case 18://jar
		...
		//execl("/usr/bin/java", "/usr/bin/java", "-jar",  "Main.jar", (char *) NULL);
		//>system("/usr/bin/java -jar Main.jar");

		execl("/usr/bin/java", "/usr/bin/java", java_xms, java_xmx,
				"-Djava.security.manager",
				"-Djava.security.policy=./java.policy", "-jar", "Main.jar", (char *) NULL);
...
//jvm gc ask VM before need,so used kernel page fault times and page size
		if (lang == 3 || lang == 7 || lang == 16 || lang==9 ||lang==17  || lang == 18) {
...
int get_sim(int solution_id, int lang, int pid, int &sim_s_id) {
    if(lang==18)return 0;//ToDo FuckThis add jar sim algorithm
	char src_pth[BUFFER_SIZE];
	//char cmd[BUFFER_SIZE];
	sprintf(src_pth, "Main.%s", lang_ext[lang]);
